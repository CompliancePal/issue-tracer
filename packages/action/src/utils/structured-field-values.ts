...
export function serializeInnerList(value) {
  return `(${value.value.map(serializeItem).join(' ')})${serializeParams(
    value.params
  )}`
}
...
export function serializeInteger(value) {
  if (value < -999_999_999_999_999n || 999_999_999_999_999n < value)
...
function roundToEven(value, precision) {
  ...
  const isEquidistant =
    Math.abs(((value * decimalShift) % 1) - 0.5) < Number.EPSILON
  if (isEquidistant) {
    // If the tail of the decimal place is 'equidistant' we round to the nearest even value
    const flooredValue = Math.floor(value * decimalShift)
    return (
      (flooredValue % 2 === 0 ? flooredValue : flooredValue + 1) / decimalShift
    )
  } else {
...
export function parseIntegerOrDecimal(input_string) {
  ...
  if (input_string[0] === '.') {
    ...
    if (output_number < -999999999999999n || 999999999999999n < output_number)
      throw new Error(
        `failed to parse integer: ${input_number} as Integer or Decimal`
      )
  }
  return {
    value: output_number,
